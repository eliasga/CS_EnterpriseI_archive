  ...
  *chartInstance)
{
  ...
  _sfTime_ = sf_get_time(chartInstance->S);
  _SFD_CC_CALL(CHART_ENTER_SFUNCTION_TAG, 6U, chartInstance->c35_sfEvent);
  for (c35_i2 = 0; c35_i2 < 3; c35_i2++) {
    _SFD_DATA_RANGE_CHECK((*c35_b_eta_dot)[c35_i2], 0U);
  }

  chartInstance->c35_sfEvent = CALL_EVENT;
  _SFD_CC_CALL(CHART_ENTER_DURING_FUNCTION_TAG, 6U, chartInstance->c35_sfEvent);
  ...
  _SFD_CC_CALL(EXIT_OUT_OF_FUNCTION_TAG, 6U, chartInstance->c35_sfEvent);
  _SFD_SYMBOL_SCOPE_POP();
