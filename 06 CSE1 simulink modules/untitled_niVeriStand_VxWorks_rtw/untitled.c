...
static void untitled_output(void)
{
   ...
   *  Gain: '<Root>/Gain'
   */
  rtb_TmpSignalConversionAtToFile[0] = untitled_B.NIVeriStandIn1;
  rtb_TmpSignalConversionAtToFile[1] = untitled_P.Gain_Gain *
    untitled_B.NIVeriStandIn1;

  /* ToFile: '<Root>/To File' */
  {
    if (!(++untitled_DW.ToFile_IWORK.Decimation % 1) &&
        (untitled_DW.ToFile_IWORK.Count*3)+1 < 100000000 ) {
      FILE *fp = (FILE *) untitled_DW.ToFile_PWORK.FilePtr;
      if (fp != (NULL)) {
        real_T u[3];
        untitled_DW.ToFile_IWORK.Decimation = 0;
        u[0] = untitled_M->Timing.t[0];
        u[1] = rtb_TmpSignalConversionAtToFile[0];
        u[2] = rtb_TmpSignalConversionAtToFile[1];
        if (fwrite(u, sizeof(real_T), 3, fp) != 3) {
          rtmSetErrorStatus(untitled_M, "Error writing to MAT-file utda.mat");
          return;
        }

        if (((++untitled_DW.ToFile_IWORK.Count)*3)+1 >= 100000000) {
          (void)fprintf(stdout,
                        "*** The ToFile block will stop logging data before\n"
                        "    the simulation has ended, because it has reached\n"
                        "    the maximum number of elements (100000000)\n"
                        "    allowed in MAT-file utda.mat.\n");
        }
      }
    }
  }
...
void untitled_initialize(void)
{
  ...
  {
    char fileName[509] = "utda.mat";
    FILE *fp = (NULL);
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(untitled_M, "Error creating .mat file utda.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,3,0,"ans")) {
      rtmSetErrorStatus(untitled_M,
                        "Error writing mat file header to file utda.mat");
      return;
    }
...
void untitled_terminate(void)
{
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "utda.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(untitled_M, "Error closing MAT-file utda.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(untitled_M, "Error reopening MAT-file utda.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 3, untitled_DW.ToFile_IWORK.Count, "ans"))
      {
        rtmSetErrorStatus(untitled_M,
                          "Error writing header for ans to MAT-file utda.mat");
      }
...
RT_MODEL_untitled_T *untitled(void)
{
  ...
  untitled_M->Sizes.numBlockPrms = (7);/* Sum of parameter "widths" */
  return untitled_M;
}
...
void SetExternalInputs(double* data, int* TaskSampleHit)
{
  ...
  // NIVeriStand In1
  if (TaskSampleHit[0]) {
    NIRT_SetValueByDataType(&untitled_B.NIVeriStandIn1, 0, data[index++], 0, 0);
  } else {
    index += 1;
  }
...
{
  { 0, "untitled/Gain/Gain", offsetof(P_untitled_T, Gain_Gain), 23, 1, 2, 0, 0 },
};
...
{
  { 0, "untitled/NIVeriStand In1", 0, "", offsetof(B_untitled_T, NIVeriStandIn1)
    +0*sizeof(real_T), BLOCKIO_SIG, 0, 1, 2, 0, 0 },

  { -1, "", -1, "", 0, 0, 0 }
};

static int NI_SigListSize DataSection(".NIVS.siglistsize") = 1;
...
{
  { 0, "NIVeriStand In1", 0, EXT_IN, 1, 1, 1 },
...
static char* NI_CompiledModelName DataSection(".NIVS.compiledmodelname") =
  "untitled";
static char* NI_CompiledModelVersion = "1.0";
static char* NI_CompiledModelDateTime = "Tue Mar 17 15:44:28 2015";
...
/* Copy the	statically initialized params into our copy of the param struct
   The undef allows us to access the real untitled_P
   In the rest of the code untitled_P is redefine to be the read-side
   of rtParameter.
 */
#undef untitled_P
...
{
  memcpy(&rtParameter[0], &untitled_P, sizeof(P_untitled_T));
  return NI_OK;
}

/*  After the model has initialized and updated the parameters in
   rtParameter[0] (aka untitled_P) we make a copy to rtParameter[1] so
...
int NI_InitParamDoubleBuf()
{
  ...
  for (; idx <= NUMST - TID01EQ; idx++)
    ...
    memcpy(&rtParameter[idx], &rtParameter[0], sizeof(P_untitled_T));
  return NI_OK;
}
...
DLL_EXPORT long NIRT_SetParameter(long index, long subindex, double value)
{
  ...
  if (index < 0) {
    ...
    memcpy(&rtParameter[1-READSIDE], &rtParameter[READSIDE], sizeof(P_untitled_T));
    WriteSideDirtyFlag = 0;
    return NI_OK;
  }
  ...
  * discStates, long* numClockTicks, char* clockTicksNames, long* clockTicks)
{
  ...
  if (discStates && discStatesNames) {
    idx = 0;
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.NIVeriStandIn1_DWORK1,
      0, 0, 0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.NIVeriStandIn1_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.ToFile_PWORK, 0, 11,
      0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.ToFile_PWORK");
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.ToFile_IWORK.Count, 0,
      19, 0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.ToFile_IWORK.Count");
    discStates[idx] = NIRT_GetValueByDataType
      (&untitled_DW.ToFile_IWORK.Decimation, 0, 19, 0);
    strcpy(discStatesNames + (idx++ * 100),
           "&untitled_DW.ToFile_IWORK.Decimation");
    for (count = 0; count < 9; count++) {
      discStates[idx] = NIRT_GetValueByDataType
        (&untitled_DW.NIVeriStandIn1_DWORK2, count, 18, 0);
      strcpy(discStatesNames + (idx++ * 100),
             "&untitled_DW.NIVeriStandIn1_DWORK2");
    }
  }
  ...
  * clockTicks)
{
  ...
  if (discStates) {
    idx = 0;
    NIRT_SetValueByDataType(&untitled_DW.NIVeriStandIn1_DWORK1, 0,
      discStates[idx++], 0, 0);
    NIRT_SetValueByDataType(&untitled_DW.ToFile_PWORK, 0, discStates[idx++], 11,
      0);
    NIRT_SetValueByDataType(&untitled_DW.ToFile_IWORK.Count, 0, discStates[idx++],
      19, 0);
    NIRT_SetValueByDataType(&untitled_DW.ToFile_IWORK.Decimation, 0,
      discStates[idx++], 19, 0);
    for (count = 0; count < 9; count++) {
      NIRT_SetValueByDataType(&untitled_DW.NIVeriStandIn1_DWORK2, count,
        discStates[idx++], 18, 0);
    }
  }
